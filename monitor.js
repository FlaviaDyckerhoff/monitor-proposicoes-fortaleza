...
const CASA_RADAR03 = process.env.CASA_RADAR03 || 'CE - Fortaleza';
...
function mlClientInterestContext() {
  ...
  await transporter.sendMail({
    ...
    to: EMAIL_DESTINO,
    subject: assuntoEmailClienteCitado(novas, `🏛️ Fortaleza: ${novas.length} nova(s) matéria(s) — ${new Date().toLocaleDateString('pt-BR')}`),
...
(async () => {
  ...
  if (CONTROLE03_FORCE_LATEST) {
    await sincronizarRadar03(novas.length ? novas : proposicoes.slice(0, 120));
